...
Browser-based local hashing utility using Web Crypto.

School Purpose Only.

## Overview

Hashpanel is a polished educational cybersecurity utility by LayerByte. It focuses on one practical defensive concept and keeps the implementation small enough for students to read, run, and understand.

Hashing is a foundational integrity concept, and browser-based hashing makes it easy to test files locally.

## Highlights

- Uses browser crypto APIs
- Keeps files local
- Shows hashes in copy-friendly form
...
- Local files selected in the browser
- Text samples where supported
...
```bash
git clone https://github.com/LayerByte/hashpanel.git
cd hashpanel
```
...
Digest values for selected local input.
